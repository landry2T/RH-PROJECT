...
<main class="app-content">
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="tile">
        <div class="tile-body">
          <h3><i class="bi bi-table"></i> Heures supplémentaires</h3>
          <div style="float:right;"> <a href="{{route('heures.create')}}" class="btn btn-primary btn-md"><i class="bi bi-plus"></i> Ajouter une heure supplementaire</a></div><br><br><br>
          ...
          <table class="table table-hover table-bordered" id="sampleTable">
            <thead>
              <tr>
                ...
                <th  class="text-center">Nombre heure</th>
                <th  class="text-center">Montant heure</th>
                <th  class="text-center">Total</th>
                <th  class="text-center">Mois de l'ajout</th>
            ...
            <tbody>
              ...
              <tr class="text-center">
                ...
                <td>{{$heure->nbre_heure}} H</td>
                <td><?php echo number_format($heure->montant_heure,2,'.',''); ?> Fcfa</td>
                <td><?php echo number_format(($heure->montant_heure*$heure->nbre_heure),2,'.',''); ?> Fcfa</td>
                <td>
                  @if($heure->mois==1)
                  janvier
                  @elseif($heure->mois==2)
                ...
                <td>{{$heure->create}}</td>
                <td><a href="#" class="btn btn-danger btn-sm" href="{{route('heures.destroy', $heure->id) }}" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $heure->id }}').submit();"data-bs-toggle="modal" data-bs-target="#staticBackdrop{{ $heure->id }}"> <i class="bi bi-trash"></i></a>

                  <a  href="{{route('heures.update', $heure->id) }}" class="btn btn-success btn-sm" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $heure->id }}').submit();"data-bs-toggle="modal" data-bs-target="#staticBackdropes{{ $heure->id }}"> <i class="bi bi-pencil"></i></a></td>
                </tr>
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      <div class="modal-footer">
                        <form method="POST" action="{{route('heures.destroy', $heure->id) }}">
                          @csrf
                          @method('DELETE')
                ...
                <div class="modal fade" id="staticBackdropes{{ $heure->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                  <div class="modal-dialog">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="staticBackdropLabel"><i class="bi bi-pencil"></i> Mettre a jour les heures de "{{$heure->Fname}}"</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                      </div>
                      <form method="POST" action="{{route('heures.update', $heure->id) }}">   
                        @csrf
                        @method('PUT')
                       ...
                       <input type="number" name="nbre" class="form-control" value="{{$heure->nbre_heure}}">

                       <label>Montant heure:*</label>
                       <input type="number" name="montant" class="form-control" value="{{$heure->montant_heure}}" min="1"><br>
                     </div>
                     <div class="modal-footer">
                       <button type="submit" class="btn btn-primary"><i class="bi bi-pencil"></i> Modifier Heure supplementaire</button>
